...
#if defined(NDEBUG)
#error "Mgpcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uMGP) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.40; // 40% of block reward
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MGP: We use certain sporks during IBD, so check to see if they are
